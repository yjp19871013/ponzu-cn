...
func uploadContentsHandler(res http.ResponseWriter, req *http.Request) {
	...
	html := `<div class="col s9 card">
					...
					<div class="col s8">
						<div class="row">
							<div class="card-title col s7">上传条目</div>
							<div class="col s5 input-field inline">
								<select class="browser-default __ponzu sort-order">
									<option value="DESC">从现在到过去</option>
									<option value="ASC">从过去到现在</option>
								</select>
					...
					<form class="col s4" action="/admin/uploads/search" method="get">
						<div class="input-field post-search inline">
							...
							<input class="search" name="q" type="text" placeholder="在上传字段范围内" class="search"/>
	...
	btn := `<div class="col s3"><a href="/admin/edit/upload" class="btn new-post waves-effect waves-light">新建上传</a></div></div>`
...
func contentsHandler(res http.ResponseWriter, req *http.Request) {
	...
	html := `<div class="col s9 card">
					...
					<div class="col s8">
						<div class="row">
							<div class="card-title col s7">` + t + ` 条目</div>
							<div class="col s5 input-field inline">
								<select class="browser-default __ponzu sort-order">
									<option value="DESC">从现在到过去</option>
									<option value="ASC">从过去到现在</option>
					...
					<form class="col s4" action="/admin/contents/search" method="get">
						<div class="input-field post-search inline">
							<label class="active">搜索:</label>
							<i class="right material-icons search-icon">search</i>
							<input class="search" name="q" type="text" placeholder="在 ` + t + ` 字段范围内" class="search"/>
	...
	if hasExt {
		...
		case "public", "":
			...
			for i := range posts {
				...
				if err != nil {
					...
					post := `<li class="col s12">数据异常，可能文件已损坏</li>`
					_, err := b.Write([]byte(post))
					if err != nil {
						log.Println(err)
		...
		case "pending":
			...
			html += `<div class="row externalable">
					<span class="description">状态:</span>
					<a href="` + publicURL + `">发布</a>
					&nbsp;&vert;&nbsp;
					<span class="active">挂起</span>
				</div>`
...
func adminPostListItem(e editor.Editable, typeName, status string) []byte {
	...
	post := `
			<li class="col s12">
				` + link + `
				<span class="post-detail">更新时间: ` + updatedTime + `</span>
...
func searchHandler(res http.ResponseWriter, req *http.Request) {
	...
	html := `<div class="col s9 card">
					<div class="card-content">
					<div class="row">
					<div class="card-title col s7">` + t + ` 结果</div>
					<form class="col s4" action="/admin/contents/search" method="get">
						<div class="input-field post-search inline">
							<label class="active">搜索:</label>
							<i class="right material-icons search-icon">search</i>
							<input class="search" name="q" type="text" placeholder="在 ` + t + ` 字段内" class="search"/>
							<input type="hidden" name="type" value="` + t + `" />
							<input type="hidden" name="status" value="` + status + `" />
						</div>
                    </form>
	...
	for i := range posts {
		...
		if err != nil {
			...
			post := `<li class="col s12">数据错误，可能文件已损坏</li>`
			_, err = b.Write([]byte(post))
			if err != nil {
				log.Println(err)
...
func addonsHandler(res http.ResponseWriter, req *http.Request) {
	...
	case http.MethodGet:
		...
		open := `<div class="col s9 card">
				<div class="card-content">
				<div class="row">
				<div class="card-title col s7">其他</div>
				</div>
				<ul class="posts row">`
